...
if __name__ == '__main__':
  ...
  for _ in range(maxIterations):
    ...
    for i in range(n):
      xi = X[i][:]
      if y[i] == 1:
        sumr = sumr + xi*(1-sigmoid(np.dot(WR,xi)))
        sumg = sumg - xi*(1-sigmoid(-np.dot(WG,xi)))
        sumb = sumb - xi*(1-sigmoid(-np.dot(WB,xi)))
      elif y[i] == 2:
        sumr = sumr - xi*(1-sigmoid(-np.dot(WR,xi)))
        sumg = sumg + xi*(1-sigmoid(np.dot(WG,xi)))
        sumb = sumb - xi*(1-sigmoid(-np.dot(WB,xi)))
        ...
        sumb = sumb + xi*(1-sigmoid(np.dot(WB,xi)))
     #elif y[i] == 2:
      #  sumr = sumr -(sigmoid(-np.dot(wr,X1))
      #elif y[i] == 2:
      #  sumg = sumg + (sigmoid(np.dot(wg,X1))
      #elif y[i]==1:
      #  sumg = sumg -(sigmoid(-np.dot(wg,X1))
      #elif y[i] == 3:
      #  sumb = sumb + (sigmoid(np.dot(wb,X1))
      #elif y[i] ==2:
      #  sumb = sumb -(sigmoid(-np.dot(wb,X1))
